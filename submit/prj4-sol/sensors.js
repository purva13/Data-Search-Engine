...
function getSearchSensorTypeForm(app) {
    ...
    try {
      ...
      const fields_SensorType = users.data.map((u) => ({id: u.id, fields: fieldsWithValues(u)}));     
        if(users.next){
            next = users.next.slice(users.next.indexOf('?'));
        }
        if(users.prev){
            prev = users.prev.slice(users.prev.indexOf('?'));;
        }
        model = { base: app.locals.base, users: fields_SensorType, fields: FIELDS, next: next, previous: prev};
...
function getSearchSensorForm(app) {
    ...
    if(users.prev){
        ...
        model = { base: app.locals.base, users: fields_Sensors, fields: FIELDS , next : next, previous : prev };
...
function fieldsWithValues(values, errors={}) {
    return FIELDS.map(function (info) {
        ...
        if(extraInfo.value === 0) extraInfo.value = "0"; //Render "0" in mustache
